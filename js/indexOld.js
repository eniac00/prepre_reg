...
async function start() {
    ...
    const res = await fetch('https://usis-cdn.eniamza.com/old-usisdump.json');
    ...
    let re = /(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\(\d{2}:\d{2}\s(AM|PM)-\d{2}:\d{2}\s(AM|PM)-[^\)]+\)/g    //Matches for Class Schedule
    let reLab =  /(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\(\d{2}:\d{2}\s(AM|PM)-\d{2}:\d{2}\s(AM|PM)(-[^\)]+)?\)/g; //Matches for Lab Schedule
    ...
    for (let i = 1; i < schedule.length; i++) {        
        ...
        if (!schedule[i]["classSchedule"]) {
            schedule[i]["classSchedule"] = [];
        }

        if (schedule[i]["LabSchedule"]) {
            let labMatches = schedule[i]["LabSchedule"].match(reLab);
            if (labMatches) {
                schedule[i]["classSchedule"].push(...labMatches); // Use spread operator to flatten the array
            }
        }
...
function info_populator (side, course_desc) {
    ...
    document.querySelector(`.${side} #time`).innerHTML = course_desc["classSchedule"];
